...
func (b *backend) pathUpdateRSAKeys(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	// var err error
	...
	if err != nil {
		logger.Log(backendLogger, config.Error, "updateRSAKey: could not get storage entry", err.Error())
	...
	if err != nil {
		logger.Log(backendLogger, config.Error, "updateRSAKey: could not get user details", err.Error())
		return nil, logical.CodedError(http.StatusUnprocessableEntity, err.Error())
	}
	...
	if err != nil {
		logger.Log(backendLogger, config.Error, "updateRSAKey: could not set storage entry", err.Error())
		return nil, logical.CodedError(http.StatusExpectationFailed, err.Error())
	}

	// put user information in store
	if err = req.Storage.Put(ctx, store); err != nil {
		logger.Log(backendLogger, config.Error, "updateRSAKey: could not put user info in storage", err.Error())
		return nil, logical.CodedError(http.StatusExpectationFailed, err.Error())
	}
	...
	return &logical.Response{
		Data: map[string]interface{}{
			"remarks": "rsa public key updated",
			"status":  true,
		},
	}, nil
}
