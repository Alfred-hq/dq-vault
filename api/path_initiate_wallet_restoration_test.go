...
func TestPathInitiateWalletRestoration(t *testing.T) {
	...
	s = mocks.NewMockStorage(ctrl)
	req.Storage = s
	s.EXPECT().Get(context.Background(), config.StorageBasePath+"EMAIL").Return(&logical.StorageEntry{}, nil)
	...
	s.EXPECT().List(context.Background(), gomock.Any()).Return([]string{"test"}, nil).AnyTimes()
	...
	s.EXPECT().Get(context.Background(), config.StorageBasePath+"MOBILE").Return(&logical.StorageEntry{}, nil).AnyTimes()
